...
float Autowall::GetDamage(C_CSPlayer* shooter, C_CSPlayer* target, const Vector& point, CCSWeaponInfo* custom_weapon, const Vector* start)
{
	// sanity checks.
	if (!shooter || !C_CSPlayer::GetLocalPlayer() || !target || target->IsDead() || target->IsDormant())
		return 0.0f;

	auto weapon = (C_WeaponCSBaseGun*)shooter->m_hActiveWeapon().Get();
	if (!weapon)
		return 0.f; //shooter doesn't have a weapon silly

	if (!weapon->GetCSWeaponData().IsValid())
		return 0.f;

	C_FireBulletData params;

	params.m_vecStart = shooter == C_CSPlayer::GetLocalPlayer() ? g_Vars.globals.m_vecFixedEyePosition : shooter->m_vecOrigin() + Vector(0.f, 0.f, 64.f);
	params.m_Player = shooter;
	params.m_TargetPlayer = target;
	params.m_bShouldSimulateShot = !!custom_weapon;
	params.m_WeaponData = !!custom_weapon ? custom_weapon : weapon->GetCSWeaponData().Xor();

	//shot is simulated from different origin
	if (start)
		params.m_vecStart = *start;

	params.m_flTraceLength = !!custom_weapon ? params.m_vecStart.Distance(point) : params.m_WeaponData->m_flWeaponRange;

	QAngle angles;
	Math::VectorAngles(point - params.m_vecStart, angles);
	Math::AngleVectors(angles, &params.m_vecDirection);

	auto damage = Autowall::FireBullets(&params);
...
float Autowall::FireBullets( Encrypted_t<C_FireBulletData> data ) {
	...
	while( data->m_flCurrentDamage > 0.f ) {
		...
		if (data->m_EnterTrace.fraction == 1.f) {
			if (data->m_bShouldSimulateShot) {
				//reached end point of trace, calculate damage as if it were a headshot
				ScaleDamage(data->m_TargetPlayer, data->m_flCurrentDamage, data->m_WeaponData->m_flArmorRatio, Hitgroup_Head);
				return true;
			}
		...
		if( pHittedPlayer != nullptr ) {
			if( bTargetIsValid && bHitgroupIsValid && pHittedPlayer->IsPlayer( ) && pHittedPlayer->m_entIndex <= Interfaces::m_pGlobalVars->maxClients && pHittedPlayer->m_entIndex > 0 ) {
				data->m_flCurrentDamage = ScaleDamage( pHittedPlayer, data->m_flCurrentDamage, data->m_WeaponData->m_flArmorRatio, data->m_bShouldSimulateShot ? Hitgroup_Head : (data->m_Weapon->m_iItemDefinitionIndex( ) == WEAPON_ZEUS ? Hitgroup_Generic : nHitGroup) );
...
//		float flArmorBonusRatio = 0.5f;
//		float flArmorRatioCalculated = flArmorRatio * 0.5f;
//		float fDamageToHealth = 10.f;
//
//		if (bHasHeavyArmor) {
//			flArmorRatioCalculated = flArmorRatio * 0.25f;
//			flArmorBonusRatio = 0.33f;
//
//			flArmorScale = 0.33f; // funny number
//
//			fDamageToHealth = (flDamage * flArmorRatioCalculated) * 0.85f;
//		}
//		else {
...
//	CGameTrace ExitTrace = { };
//	if (!TraceToExit(&data->m_EnterTrace, data->m_EnterTrace.endpos, data->m_vecDirection, &ExitTrace)
//		&& !(Interfaces::m_pEngineTrace->GetPointContents(data->m_EnterTrace.endpos, MASK_SHOT_HULL, 0) & MASK_SHOT_HULL)
//		|| bCannotPenetrate)
//		return true;
//
//	const surfacedata_t* pExitSurfaceData = Interfaces::m_pPhysSurface->GetSurfaceData(ExitTrace.surface.surfaceProps);
...
//		return true;
//
//	float flEnterPenetrationModifier = data->m_EnterSurfaceData->game.flPenetrationModifier;
...
//				goto LABEL_51;
//
//			if (iEnterMaterial != CHAR_TEX_GRATE)
//			{
//				C_CSPlayer* player = (C_CSPlayer*)data->m_EnterTrace.hit_entity;
//				if (player)
//				{
//					if (player->IsPlayer())
//					{
//						if (player->m_bIsPlayerGhost())
//							goto LABEL_45;
//					}
//				}
//
//				const float ff_reduction_bullets = g_Vars.ff_damage_reduction_bullets->GetFloat();
//				const float ff_damage_bullet_pen = g_Vars.ff_damage_bullet_penetration->GetFloat();
//				if (iEnterMaterial != CHAR_TEX_FLESH || ff_reduction_bullets != 0.0f || !player || !player->IsPlayer() || player->IsTeammate(C_CSPlayer::GetLocalPlayer()))
//				{
//					flPenetrationModifier = (flEnterPenetrationModifier + flExitPenetrationModifier) * 0.5f;
//					goto LABEL_46;
//				}
//
//				if (ff_damage_bullet_pen == 0.0f)
//					return true;
//
//				flEnterPenetrationModifier = ff_damage_bullet_pen;
//
//			LABEL_45:
//				flPenetrationModifier = flEnterPenetrationModifier;
//			LABEL_46:
//				flDamageModifier = 0.16f;
//				goto LABEL_52;
//			}
...
//
//		float flThickness = (ExitTrace.endpos - data->m_EnterTrace.endpos).Length();
//		if (sqrt(flThickness) <= flPenetrationModifier * data->m_iPenetrationCount)
//		{
//			data->m_flCurrentDamage *= flDamageModifier;
//			data->m_vecStart = ExitTrace.endpos;
//			--data->m_iPenetrationCount;
...
//
//float Autowall::FireBullets(Encrypted_t<C_FireBulletData> data) {
//	constexpr float rayExtension = 30.f;
//
//	//This gets set in FX_Firebullets to 4 as a pass-through value.
//	//CS:GO has a maximum of 4 surfaces a bullet can pass-through before it 100% stops.
//	//Excerpt from Valve: https://steamcommunity.com/sharedfiles/filedetails/?id=275573090
...
//
//				g_Vars.globals.m_InHBP = false;
//				return data->m_flCurrentDamage; // TOTALLY 800 DMG
//			}
//		}
//
//		bool bCanPenetrate = data->m_bPenetration;
