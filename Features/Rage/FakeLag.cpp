...
namespace Interfaces
{
	...
	public:
		...
		virtual void SetMaxFakelagChoke(int maxChoke) const {
			fakelagData.Xor()->m_iMaxChoke = maxChoke;
		}
	...
	bool C_FakeLag::AlternativeCondition(Encrypted_t<CUserCmd> cmd, bool* bSendPacket) {
		C_CSPlayer* local = C_CSPlayer::GetLocalPlayer();
		if (!local || local->IsDead() || g_Vars.fakelag.choke_type != 3 || (int)g_Vars.fakelag.alternative_choke <= 0) {
			fakelagData->m_bAlternative = false;
			return false;
		}
		...
		//if( g_Vars.fakelag.trigger_while_breaking_lc) {
		...
		if (!fakelagData->m_bAlternative) {
			...
			else if (g_Vars.fakelag.trigger_on_velocity_change && VelocityChange(cmd)) {
				fakelagData->m_bAlternative = true;
			}
			else if (g_Vars.fakelag.trigger_while_breaking_lc && BreakLagComp((int)g_Vars.fakelag.alternative_choke)) {
				fakelagData->m_bAlternative = true;
			}
			else if (g_Vars.fakelag.trigger_on_peek && Interfaces::FakeLag::Get()->IsPeeking(cmd)) {
				fakelagData->m_bAlternative = true;
				g_Vars.globals.m_bIsFakeLaggingOnPeek = true;
			}
			else if (g_Vars.fakelag.trigger_on_hit && g_Vars.globals.m_bLocalPlayerHarmedThisTick) {
				fakelagData->m_bAlternative = true;
			}
			else if (g_Vars.fakelag.trigger_shooting && cmd->buttons & IN_ATTACK) {
				fakelagData->m_bAlternative = true;
			}
			else if (g_Vars.fakelag.trigger_on_lby_update && g_Vars.globals.m_bLbyUpdated) {
				fakelagData->m_bAlternative = true;
			}
	...
	bool C_FakeLag::MainCondition(Encrypted_t<CUserCmd> cmd, bool* bSendPacket) {
		...
		bool unducking = local->m_flDuckAmount() >= 0.125f && g_Vars.globals.m_flPreviousDuckAmount > local->m_flDuckAmount();
		...
		if (!moving && !unducking)
			...
			return false;
		else if (unducking && !g_Vars.fakelag.on_unduck)
			return false;
