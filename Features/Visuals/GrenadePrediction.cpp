...
void CGrenadePrediction::Paint( ) {
	...
	if( ( wpn_index ) && vecPath.size( ) > 1 && pWeaponData->m_iWeaponType == WEAPONTYPE_GRENADE && pWeapon->m_fThrowTime( ) <= 0 ) {
		...
		if( target.second ) {
			...
			has_target_dmg = false;
		}

		// path.
		for( auto it = vecPath.begin( ), end = vecPath.end( ); it != end; ++it ) {
			if( WorldToScreen( prev, ab ) && WorldToScreen( *it, cd ) ) {
				Render::Engine::Line( ab, cd, has_target_dmg ? Color(0, 255, 0, 255) : Color(255, 0, 0, 255)/*g_Vars.esp.nade_pred_color.ToRegularColor( )*/);

				// hmm?
				Render::Engine::Line(ab + Vector2D(1, 0), cd + Vector2D(1, 0), has_target_dmg ? Color(0, 255, 0, 255) : Color(255, 0, 0, 255)/*g_Vars.esp.nade_pred_color.ToRegularColor( )*/);
			}
