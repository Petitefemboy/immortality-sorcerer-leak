...
void DrawWatermark() {
	...
	std::string text = XorStr("immortal");
...
void CEsp::Indicators() {
	...
	if (g_Vars.esp.indicators) 
	{
		if (g_Vars.rage.key_dmg_override.enabled)
		{
			...
			ind.text = XorStr("DMG");//std::to_string(rbot->min_damage_override_amount);
...
void CEsp::Keybinds() {
	std::vector<
		std::tuple<std::string, int, Color>
	> vecNames;
	...
	auto AddBind = [this, &vecNames](const char* name, KeyBind_t& bind, Color color) {
		if (!bind.enabled)
			return;
		...
		vecNames.push_back(std::tuple<std::string, int, Color>(std::string(name), bind.cond, color));
	};

	if (g_Vars.rage.enabled) {
		//AddBind( XorStr( "Force safe point" ), g_Vars.rage.force_safe_point );
		AddBind(XorStr("Damage Override"), g_Vars.rage.key_dmg_override, Color(255, 255, 255));
		AddBind(XorStr("Force Body Aim"), g_Vars.rage.prefer_body, Color(255, 255, 255));
	}
	...
	AddBind(XorStr("Hitscan Override"), g_Vars.rage.override_key, Color(255, 255, 255));

	if (g_Vars.misc.edgejump) {
		AddBind(XorStr("Edge Jump"), g_Vars.misc.edgejump_bind, Color(255, 255, 255));
	}

	if (g_Vars.misc.autopeek) {
		AddBind(XorStr("Auto Peek"), g_Vars.misc.autopeek_bind, Color(255, 255, 255));
	}

	if (g_Vars.misc.slow_walk) {
		AddBind(XorStr("Fake Walk"), g_Vars.misc.slow_walk_bind, Color(255, 255, 255));
	}
	...
	if (!vecNames.empty()) {
		...
		for (auto name : vecNames) {
			// hotkey name
			Render::Engine::segoe.string(pos.x + 2, pos.y + 9 + offset, std::get<2>(name).OverrideAlpha(255 * alpha), std::get<0>(name).c_str());

			// hotkey type
			Render::Engine::segoe.string(pos.x + (this->m_KeyBinds.size.x - (std::get<1>(name) == KeyBindType::HOLD ? hold_size.m_width : std::get<1>(name) == KeyBindType::TOGGLE ? toggle_size.m_width : always_size.m_width)), pos.y + 9 + offset, std::get<2>(name).OverrideAlpha(255 * alpha),
				std::get<1>(name) == KeyBindType::HOLD ? XorStr("Hold") : std::get<1>(name) == KeyBindType::TOGGLE ? XorStr("Toggle") : XorStr("Always"));
...
void CEsp::Main() {
	...
	m_LocalPlayer = C_CSPlayer::GetLocalPlayer();      // unassigned team check ↓
	if (!g_Vars.globals.HackIsReady || (!m_LocalPlayer || m_LocalPlayer->m_iTeamNum() == 0) || !Interfaces::m_pEngine->IsInGame()) // you know it's smart to check if you are in game too right!??!
		return;
...
void CEsp::DrawInfo(C_CSPlayer* player, BBox_t bbox, player_info_t player_info) {
	...
	if (&player->m_AnimOverlay().Element(6) && !player->IsDormant() && g_Vars.esp.draw_anim_layers) {
		g_Vars.globals.m_vecTextInfo[player->EntIndex()].emplace_back(FloatColor(255, 255, 255, (int)(180 * m_flAlpha[player->EntIndex()])), std::to_string(player->m_AnimOverlay().Element(6).m_flWeight));
		g_Vars.globals.m_vecTextInfo[player->EntIndex()].emplace_back(FloatColor(255, 255, 255, (int)(180 * m_flAlpha[player->EntIndex()])), std::to_string(player->m_AnimOverlay().Element(6).m_flCycle));
		g_Vars.globals.m_vecTextInfo[player->EntIndex()].emplace_back(FloatColor(255, 255, 255, (int)(180 * m_flAlpha[player->EntIndex()])), std::to_string(player->m_AnimOverlay().Element(6).m_flPrevCycle));
		g_Vars.globals.m_vecTextInfo[player->EntIndex()].emplace_back(FloatColor(255, 255, 255, (int)(180 * m_flAlpha[player->EntIndex()])), std::to_string(player->m_AnimOverlay().Element(6).m_flPlaybackRate));
		g_Vars.globals.m_vecTextInfo[player->EntIndex()].emplace_back(FloatColor(255, 255, 255, (int)(180 * m_flAlpha[player->EntIndex()])), std::to_string(player->m_AnimOverlay().Element(6).m_flWeightDeltaRate));
	}
