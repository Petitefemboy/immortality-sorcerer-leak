...
namespace Interfaces
{
	...
	void C_Movement::InPrediction() {
		...
		*(float*)(uintptr_t(m_movement_data->m_pLocal) + Engine::Displacement.DT_BaseAnimating.m_flPoseParameter + 48) = PitchPosBackup;
		...
		//if (!g_TickbaseController.m_bIncrementTicksAllowedForProcessing)
	...
	void C_Movement::SlowWalk(float speed, bool force_accurate) {
		...
		if (g_Vars.globals.m_iTicksAllowed) {
			g_Vars.globals.Fakewalking = false;
			return;
		}
		...
		*m_movement_data->m_pSendPacket = Interfaces::m_pClientState->m_nChokedCommands() > g_Vars.misc.slow_walk_speed;
		...
		//{
		//	if (g_Vars.globals.m_flBodyPred > 0.15)
		//		*m_movement_data->m_pSendPacket = true;
		//	else
		...
		//if (m_movement_data->m_pSendPacket)
		//{
		//	const int choked_ticks = Interfaces::m_pClientState->m_nChokedCommands();

		//	if (!choked_ticks || choked_ticks > (animstate->m_velocity_length_xy < 115.f ? 9 : 8))
		//	{
		//		m_movement_data->m_pCmd->sidemove = 0;
		//		m_movement_data->m_pCmd->forwardmove = 450;

		//		if (!choked_ticks || animstate->m_velocity_length_xy < 20.f)
		//			m_movement_data->m_pCmd->forwardmove = 0;

		//		RotateMovement(m_movement_data->m_pCmd.Xor(), Math::CalcAngle(Vector(0, 0, 0), LocalPlayer->m_vecVelocity()).y + 180.f);
		//	}
		//}
		//else
		//{
		...
		if (Interfaces::m_pGlobalVars->curtime >= g_Vars.globals.m_flBodyPred || Interfaces::m_pClientState->m_nChokedCommands() > (g_Vars.misc.slow_walk_speed - nTicksToStop) || !Interfaces::m_pClientState->m_nChokedCommands() || *m_movement_data->m_pSendPacket) {
