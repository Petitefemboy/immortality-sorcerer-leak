...
namespace Engine
{
	...
	void Prediction::Begin(Encrypted_t<CUserCmd> _cmd, bool* send_packet, int command_number) {
		...
		*reinterpret_cast<CUserCmd**>(uint32_t(predictionData->m_pPlayer) + 0x326C) = predictionData->m_pCmd.Xor(); // unk01
		...
		//g_TickbaseController.SetCurrentCommandTickbase();
	...
	void Prediction::KeepCommunication(bool* bSendPacket, int command_number) {
		auto local = C_CSPlayer::GetLocalPlayer();
		if (!local || local->IsDead()) {
			g_Vars.globals.cmds.clear(); // -- possibly uncomment this.
			return;
		}
	...
	inline bool IsFloatValidHigherTol(float_t flOriginal, float_t flCurrent) {
		if (fabsf(flOriginal - flCurrent) <= 0.04f)
			return false;

		return true;
	}

	void Prediction::ApplyCompressionFix() {
	//	if (g_Vars.globals.m_iFrameStage != FRAME_NET_UPDATE_POSTDATAUPDATE_START)
	//		return;
		...
		if (!IsAngleValid(local->m_aimPunchAngle(), data->m_aimPunchAngle)) {
			local->m_aimPunchAngle() = data->m_aimPunchAngle;
			m_bNetvarCompressionFixHasBeenApplied[0] = true;
		}
		else // set this back to false
			m_bNetvarCompressionFixHasBeenApplied[0] = false;

		if (!IsAngleValid(local->m_aimPunchAngleVel(), data->m_aimPunchAngleVel)) {
			local->m_aimPunchAngleVel() = data->m_aimPunchAngleVel;
			m_bNetvarCompressionFixHasBeenApplied[1] = true;
		}
		else
			m_bNetvarCompressionFixHasBeenApplied[1] = false;

		if (!IsVectorValid(local->m_vecBaseVelocity(), data->m_vecBaseVelocity)) {
			local->m_vecBaseVelocity() = data->m_vecBaseVelocity;
			m_bNetvarCompressionFixHasBeenApplied[2] = true;
		}
		else
			m_bNetvarCompressionFixHasBeenApplied[2] = false;

		if (!IsVectorValid(local->m_vecViewOffset(), data->m_vecViewOffset)) {
			local->m_vecViewOffset() = data->m_vecViewOffset;
			m_bNetvarCompressionFixHasBeenApplied[3] = true;
		}
		else
			m_bNetvarCompressionFixHasBeenApplied[3] = false;

		//if (!IsFloatValid(local->m_flVelocityModifier(), data->m_flVelocityModifier)) {
		//	local->m_flVelocityModifier() = data->m_flVelocityModifier;
		//	m_bNetvarCompressionFixHasBeenApplied[4] = true;
		//}
		//else
		//	m_bNetvarCompressionFixHasBeenApplied[4] = false;

		if (!IsFloatValidHigherTol(local->m_flFallVelocity(), data->m_flFallVelocity)) {
			local->m_flFallVelocity() = data->m_flFallVelocity;
			m_bNetvarCompressionFixHasBeenApplied[5] = true;
		}
		else
			m_bNetvarCompressionFixHasBeenApplied[5] = false;

		if (!IsFloatValid(local->m_flDuckAmount(), data->m_flDuckAmount)) {
			local->m_flDuckAmount() = data->m_flDuckAmount;
			m_bNetvarCompressionFixHasBeenApplied[6] = true;
		}
		else
			m_bNetvarCompressionFixHasBeenApplied[6] = false;

		if (!IsFloatValid(local->m_flDuckSpeed(), data->m_flDuckSpeed)) {
			local->m_flDuckSpeed() = data->m_flDuckSpeed;
			m_bNetvarCompressionFixHasBeenApplied[7] = true;
		}
		else
			m_bNetvarCompressionFixHasBeenApplied[7] = false;

		static int playerSurfaceFrictionOffset = Memory::FindInDataMap(local->GetPredDescMap(), XorStr("m_surfaceFriction"));
		float playerSurfaceFriction = *(float*)(uintptr_t(local) + playerSurfaceFrictionOffset);
		if (!IsFloatValid(playerSurfaceFriction, data->m_surfaceFriction)) {
			playerSurfaceFriction = data->m_surfaceFriction;
			m_bNetvarCompressionFixHasBeenApplied[8] = true;
		}
		else
			m_bNetvarCompressionFixHasBeenApplied[8] = false;

		auto weapon = (C_WeaponCSBaseGun*)local->m_hActiveWeapon().Get();
		if (weapon) {
			static int fAccuracyPenalty = Memory::FindInDataMap(local->GetPredDescMap(), XorStr("m_fAccuracyPenalty"));
			float playerfAccuracyPenalty = *(float*)(uintptr_t(local) + fAccuracyPenalty);
			if (!IsFloatValid(playerfAccuracyPenalty, data->m_fAccuracyPenalty)) {
				playerfAccuracyPenalty = data->m_fAccuracyPenalty;
				m_bNetvarCompressionFixHasBeenApplied[9] = true;
			}
			else
				m_bNetvarCompressionFixHasBeenApplied[9] = false;

			if (!IsFloatValid(weapon->m_flRecoilIndex(), data->m_flRecoilIndex)) {
				weapon->m_flRecoilIndex() = data->m_flRecoilIndex;
				m_bNetvarCompressionFixHasBeenApplied[10] = true;
			}
			else
				m_bNetvarCompressionFixHasBeenApplied[10] = false;

			if (!IsFloatValid(weapon->m_flNextPrimaryAttack(), data->m_flNextPrimaryAttack)) {
				weapon->m_flNextPrimaryAttack() = data->m_flNextPrimaryAttack;
				m_bNetvarCompressionFixHasBeenApplied[14] = true;
			}
			else
				m_bNetvarCompressionFixHasBeenApplied[14] = false;

			if (!IsFloatValid(weapon->m_flNextSecondaryAttack(), data->m_flNextSecondaryAttack)) {
				weapon->m_flNextSecondaryAttack() = data->m_flNextSecondaryAttack;
				m_bNetvarCompressionFixHasBeenApplied[15] = true;
			}
			else
				m_bNetvarCompressionFixHasBeenApplied[15] = false;

			if (!IsFloatValid(weapon->m_flPostponeFireReadyTime(), data->m_flPostponeFireReadyTime)) {
				weapon->m_flPostponeFireReadyTime() = data->m_flPostponeFireReadyTime;
				m_bNetvarCompressionFixHasBeenApplied[16] = true;
			}
			else
				m_bNetvarCompressionFixHasBeenApplied[16] = false;
		}

		if (!IsAngleValid(local->m_viewPunchAngle(), data->m_viewPunchAngle)) {
			local->m_viewPunchAngle() = data->m_viewPunchAngle;
			m_bNetvarCompressionFixHasBeenApplied[11] = true;
		}
		else
			m_bNetvarCompressionFixHasBeenApplied[11] = false;

		if (!IsVectorValid(local->m_vecVelocity(), data->m_vecVelocity)) {
			local->m_vecVelocity() = data->m_vecVelocity;
			m_bNetvarCompressionFixHasBeenApplied[12] = true;
		}
		else
			m_bNetvarCompressionFixHasBeenApplied[12] = false;

		if (!IsVectorValid(local->GetNetworkOrigin(), data->m_vecNetworkOrigin)) {
			local->GetNetworkOrigin() = data->m_vecNetworkOrigin;
			m_bNetvarCompressionFixHasBeenApplied[13] = true;
		}
		else
			m_bNetvarCompressionFixHasBeenApplied[13] = false;

		if (!IsVectorValid(local->m_vecOrigin(), data->m_vecOrigin)) {
			local->m_vecOrigin() = data->m_vecOrigin;
			m_bNetvarCompressionFixHasBeenApplied[17] = true;
		}
		else
			m_bNetvarCompressionFixHasBeenApplied[17] = false;

		if (!m_bNetvarCompressionFixHasBeenApplied)    
			m_bNetvarCompressionWasntApplied = true; // everything is fine!
		else
			m_bNetvarCompressionWasntApplied = false;
	...
	void Prediction::StoreNetvarCompression(CUserCmd* cmd)
	{
		...
		data->m_vecNetworkOrigin = local->GetNetworkOrigin();
		...
		data->m_surfaceFriction = playerSurfaceFriction;
		...
		if (weapon) {
			...
			data->m_fAccuracyPenalty = playerfAccuracyPenalty;
	...
	// i was hoping by doing this here we wouldn't have to repredict to fix our issues as this is the data we will use to finish our prediction
	// i was also hoping that it would also prevent this from overwriting the netvar compression fix.
	void RestoreData::Setup(C_CSPlayer* player) {
		// did this so i don't have to constantly fucking type it out
		auto predictionPtr = &Engine::Prediction::Instance();
		...
		/*auto slot = player->m_nTickBase();
		auto data = &predictionPtr->predictionData->m_Data[slot % 150];
		if (!data) {*/ // just in case we haven't grabbed prediction data yet.
		...
		//// we have grabbed our prediction data, we can apply and do our shit based off of this
		//else {
		//	// all of our data is valid, we can just set our data to the prediction data that is currently set to the entity
		//	if (predictionPtr->m_bNetvarCompressionWasntApplied) {
		//		this->m_aimPunchAngle = player->m_aimPunchAngle();
		//		this->m_aimPunchAngleVel = player->m_aimPunchAngleVel();
		//		this->m_viewPunchAngle = player->m_viewPunchAngle();
		...
		//	// somewhere our data is not valid, decypher where it is.
		//	else {
		//		if (predictionPtr->m_bNetvarCompressionFixHasBeenApplied[0])
		//			this->m_aimPunchAngle = data->m_aimPunchAngle;
		//		else
		//			this->m_aimPunchAngle = player->m_aimPunchAngle();

		//		if (predictionPtr->m_bNetvarCompressionFixHasBeenApplied[1])
		//			this->m_aimPunchAngleVel = data->m_aimPunchAngleVel;
		//		else
		//			this->m_aimPunchAngleVel = player->m_aimPunchAngleVel();

		//		if (predictionPtr->m_bNetvarCompressionFixHasBeenApplied[2])
		//			this->m_vecBaseVelocity = data->m_vecBaseVelocity;
		//		else
		//			this->m_vecBaseVelocity = player->m_vecBaseVelocity();

		//		if (predictionPtr->m_bNetvarCompressionFixHasBeenApplied[3])
		//			this->m_vecViewOffset = data->m_vecViewOffset;
		//		else
		//			this->m_vecViewOffset = player->m_vecViewOffset();

		//		//if (predictionPtr->m_bNetvarCompressionFixHasBeenApplied[4])
		//		//	this->m_flVelocityModifier = data->m_flVelocityModifier;
		//		//else
		//		//	this->m_flVelocityModifier = player->m_flVelocityModifier();

		//		if (predictionPtr->m_bNetvarCompressionFixHasBeenApplied[5])
		//			this->m_flFallVelocity = data->m_flFallVelocity;
		//		else
		//			this->m_flFallVelocity = player->m_flFallVelocity();

		//		if (predictionPtr->m_bNetvarCompressionFixHasBeenApplied[6])
		//			this->m_flDuckAmount = data->m_flDuckAmount;
		//		else
		//			this->m_flDuckAmount = player->m_flDuckAmount();

		//		if (predictionPtr->m_bNetvarCompressionFixHasBeenApplied[7])
		//			this->m_flDuckSpeed = data->m_flDuckSpeed;
		//		else
		//			this->m_flDuckSpeed = player->m_flDuckSpeed();

		//		if (predictionPtr->m_bNetvarCompressionFixHasBeenApplied[8])
		//			this->m_surfaceFriction = data->m_surfaceFriction;
		//		else
		//			this->m_surfaceFriction = playerSurfaceFriction;

		//		auto weapon = (C_WeaponCSBaseGun*)player->m_hActiveWeapon().Get();
		//		if (weapon) {
		//			if (predictionPtr->m_bNetvarCompressionFixHasBeenApplied[9])
		//				this->m_fAccuracyPenalty = data->m_fAccuracyPenalty;
		//			else
		//				this->m_fAccuracyPenalty = playerfAccuracyPenalty;

		//			if (predictionPtr->m_bNetvarCompressionFixHasBeenApplied[10])
		//				this->m_flRecoilIndex = data->m_flRecoilIndex;
		//			else
		//				this->m_flRecoilIndex = weapon->m_flRecoilIndex();
		//		}

		//		if (predictionPtr->m_bNetvarCompressionFixHasBeenApplied[11])
		//			this->m_viewPunchAngle = data->m_viewPunchAngle;
		//		else
		//			this->m_viewPunchAngle = player->m_viewPunchAngle();

		//		if (predictionPtr->m_bNetvarCompressionFixHasBeenApplied[12])
		//			this->m_vecVelocity = data->m_vecVelocity;
		//		else
		//			this->m_vecVelocity = player->m_vecVelocity();

		//		if (predictionPtr->m_bNetvarCompressionFixHasBeenApplied[17])
		//			this->m_vecOrigin = data->m_vecOrigin;
		//		else
		//			this->m_vecOrigin = player->m_vecOrigin();
		//	}
		//}
