...
using FnProcessMovement = void(__thiscall*)(void*, int, CMoveData*);
FnProcessMovement oProcessMovement;
void __fastcall hkProcessMovement(void* pThis, void* edx, int player, CMoveData* moveData) {
...
void __fastcall hkPhysicsSimulate(void* ecx, void* edx) {
	...
	if (!entity || !local || local->EntIndex() != entity->EntIndex() || entity->IsDead())
		return oPhysicsSimulate(ecx);

	int nSimulationTick = *(int*)(uintptr_t(entity) + 0x2AC);
	auto pCommandContext = (C_CommandContext*)(uintptr_t(entity) + 0x34FC);
...
int __fastcall TransferData(void* edx, void* ecx, const char* name, int entIndex, datamap_t* map) {
	Engine::Prediction::Instance()->ApplyCompressionFix();
	return TransferDataOriginal(edx, ecx, name, entIndex, map);
}
...
namespace Interfaces
{
	...
	bool Create(void* reserved) {
		...
		oCreateMove = Hooked::HooksManager.CreateHook<decltype(oCreateMove)>(&Hooked::CreateMove, (void*)Memory::Scan(XorStr("client.dll"), XorStr("55 8B EC 83 E4 F8 81 EC A4 01 00 00 8B 45 08 89 0C 24 53 56 57 85 C0 74 0C 50 81 C1 B4 00 00 00 E8")));
#else
		...
		//oLevelInitPreEntity = Hooked::HooksManager.HookVirtual<decltype(oLevelInitPreEntity)>(m_pClient.Xor(), &LevelInitPreEntity, Index::IBaseClientDLL::LevelInitPreEntity);
		oFrameStageNotify = Hooked::HooksManager.HookVirtual<decltype(oFrameStageNotify)>(m_pClient.Xor(), &Hooked::FrameStageNotify, Index::IBaseClientDLL::FrameStageNotify);

		//static auto CBaseEntityPostNetworkDataReceived = reinterpret_cast<void*>(Memory::Scan(XorStr("client.dll"), XorStr("E8 ?? ?? ?? ?? 33 F6 6A 02")));
		//oPostNetworkDataReceived = HooksManager.CreateHook<decltype(oPostNetworkDataReceived)>(&PostDataNetworkReceived, CBaseEntityPostNetworkDataReceived);
		...
		//static auto CCSPlayerPreDataUpdate = reinterpret_cast<void*>(Memory::Scan(XorStr("client.dll"), XorStr("55 8B EC 56 8B 75 08 8B D6 57 8B F9 8D")));
		//oPreDataUpdate = Hooked::HooksManager.CreateHook<decltype(oPreDataUpdate)>(&PreDataUpdate, CCSPlayerPreDataUpdate);
		//
		//static auto CCSPlayerPostDataUpdate = reinterpret_cast<void*>(Memory::Scan(XorStr("client.dll"), XorStr("55 8B EC 53 56 8B F1 57 80 BE ? ? ? ? ? 74 0A")));
		//oPostDataUpdate = Hooked::HooksManager.CreateHook<decltype(oPostDataUpdate)>(&PostDataUpdate, CCSPlayerPostDataUpdate);
		...
		auto cl_move = (void*)Engine::Displacement.Data.m_uCLMove;
