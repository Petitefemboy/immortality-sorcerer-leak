...
// i don't think you can predefine an enum.
...
public:
	typedef struct _GLOBAL {
		...
		bool m_bApplyTickbaseShift = false;
		bool m_bCanShiftShot = false;
		bool m_bWaitForTickbaseBeforeFiring = false;
		bool m_bPredictionStateIsShifted = false;
		bool m_bLastShotWasShifted = false;
		bool m_bIsFakeLaggingOnPeek = false;
		bool m_bIsTeleporting = false;
		bool m_bWeaponWillFireBurstShotThisTick = false;
		...
		bool m_bAimbotIsTargettingPlayer = false;
		...
		// double tap shit
		int m_iTicksAllowed;
		int m_iNextTicksToShift;
		int m_iTicksShifted;
		bool m_bIsDTAiming = false;
		bool m_bShouldStop = false;

		int m_iFixedTickbase;
...
#pragma region Rage general group
	...
	// new doubletap shit.
	config_option(bool, doubletap, false);
	config_keybind(key_dt);
	config_option(int, double_tap_ticks, 0);
	config_option(int, doubletapmode, 0);

	config_option(bool, breaklagcomp, false);
	config_keybind(break_key);
	config_option(int, breaklcmode, 0);

	// used with previous dt system that didn't want to run;
	// probably because cl_sendmove sig is wrong.
	config_option(bool, hide_shot_record, false);

	config_option(bool, target_limit, false);
...
#pragma region Antiaim group
	...
	config_option(bool, safebreak, false);
	config_option(bool, funnymode, false);
	config_option(bool, preserve, false);

	config_option(bool, fakeflick, false);
	config_keybind(fake_flick);
...
#pragma region Fakelag group
	...
	config_option(int, trigger_limit, 0);
	...
	config_option(bool, on_unduck, false);
	...
	config_option(bool, trigger_on_hit, false);
	config_option(bool, trigger_reloading, false);
	config_option(bool, trigger_on_peek, false);
	config_option(bool, trigger_while_breaking_lc, false);
	config_option(bool, trigger_on_velocity_change, false);
	config_option(bool, trigger_on_lby_update, false);
...
#pragma region ESP group
	...
	config_option(bool, draw_anim_layers, false);
...
#pragma region MISC group
	...
	config_option(int, cheat_mode, 0);
