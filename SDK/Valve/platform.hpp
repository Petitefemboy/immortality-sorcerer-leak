...
__forceinline bool IsFPUControlWordSet( ) {
  float f = 0.996f;
  union
  {
	 double flResult;
...
#ifdef COMPILER_MSVC
__forceinline uint32_t RotateBitsLeft32( uint32_t nValue, int nRotateBits ) {
  return _rotl( nValue, nRotateBits );
}
__forceinline uint64_t RotateBitsLeft64( uint64_t nValue, int nRotateBits ) {
  return _rotl64( nValue, nRotateBits );
}
__forceinline uint32_t RotateBitsRight32( uint32_t nValue, int nRotateBits ) {
  return _rotr( nValue, nRotateBits );
}
__forceinline uint64_t RotateBitsRight64( uint64_t nValue, int nRotateBits ) {
