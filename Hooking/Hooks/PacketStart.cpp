...
void __fastcall Hooked::PacketEnd( void* ecx, void* ) {
	...
	if (Interfaces::m_pClientState->m_ClockDriftMgr.m_nServerTick != Interfaces::m_pClientState->m_nDeltaTick()) {
		oPacketEnd(ecx);
		return;
	}

	// How many commands total did we run this frame
	int commands_acknowledged = Interfaces::m_pClientState->m_nCommandAck() - Interfaces::m_pClientState->m_nLastCommandAck();
