...
namespace Hooked
{
	...
	void UpdateInformation(CUserCmd* cmd) {
		...
		// fix landing anim.
		if( state->m_on_ground && !state->m_smooth_height_valid && state->m_anim_duck_amount > 0.f )
			g_Vars.globals.RegularAngles.x = -7.5f;
		...
		if (state->m_on_ground) {
			...
			if (state->m_velocity_length_xy > 0.1f) {
				...
				if (g_Vars.globals.m_flAnimTime > g_Vars.globals.m_flBodyPred && abs(AngleDiff(state->m_abs_yaw, state->m_eye_yaw)) > 35.0f) {
					g_Vars.globals.m_flBodyPred = g_Vars.globals.m_flAnimTime + CSGO_ANIM_LOWER_REALIGN_DELAY;
					g_Vars.globals.RegularAngles.y = Interfaces::AntiAimbot::Get()->m_flLowerBodyUpdateYaw;//g_Vars.globals.m_flBody = Interfaces::AntiAimbot::Get()->m_flLowerBodyUpdateYaw;
				}
			}
		}
		...
		// build bones at the end of everything
		{
			...
			g_Vars.globals.flRealYaw = g_Vars.globals.RegularAngles.y;
	...
	bool __stdcall CreateMove(float ft, CUserCmd* _cmd) {
		...
		g_Vars.globals.bSendPacket = reinterpret_cast<bool*>(uintptr_t(pAddrOfRetAddr.Xor()) + 0x14);
