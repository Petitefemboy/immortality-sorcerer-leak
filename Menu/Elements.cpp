...
namespace Menu {
	...
	void DrawMenu() {
		if (GUI::Form::BeginWindow(XorStr("immortal")) || GUI::ctx->setup) {
			...
			if (GUI::Form::BeginTab(0, XorStr("A")) || GUI::ctx->setup) {
				...
				GUI::Controls::Checkbox(XorStr("Limit Targets"), &g_Vars.rage.target_limit);
				...
				GUI::Controls::Checkbox(XorStr("Automatic fire"), &g_Vars.rage.auto_fire);
				{
					...
					if (rbot->active || GUI::ctx->setup) {
						...
						GUI::Controls::Checkbox(XorStr("Intensive Seed Calculation") + std::string(XorStr("#") + std::to_string(rage_current_group)), &rbot->static_point_scale);
				...
				GUI::Group::BeginGroup(XorStr("Fake lag"), Vector2D(50, 40));
				{
					...
					GUI::Controls::Dropdown(XorStr(""), { XorStr("Maximum"), XorStr("Dynamic"), XorStr("Fluctuate")/*, XorStr("Alternative")*/ }, &g_Vars.fakelag.choke_type);

					std::vector<MultiItem_t> fakelag_cond = {
						{ XorStr("Moving"), &g_Vars.fakelag.when_moving },
						{ XorStr("In air"), &g_Vars.fakelag.when_air },
						{ XorStr("On Unduck"), &g_Vars.fakelag.on_unduck },
					};
					...
					//if (g_Vars.fakelag.choke_type == 3) {
					//	std::vector<MultiItem_t> alt_fakelag_cond = {
					//		{ XorStr("On Duck"), &g_Vars.fakelag.trigger_duck },
					//		{ XorStr("On Land"), &g_Vars.fakelag.trigger_land },
					//		{ XorStr("On Weapon Change"), &g_Vars.fakelag.trigger_weapon_activity },
					//		{ XorStr("On Shot"), &g_Vars.fakelag.trigger_shooting },
					//		{ XorStr("On Hit"), &g_Vars.fakelag.trigger_on_hit },
					//		{ XorStr("On Reload"), &g_Vars.fakelag.trigger_reloading },
					//		{ XorStr("On Peek"), &g_Vars.fakelag.trigger_on_peek },
					//		{ XorStr("On Velocity Change"), &g_Vars.fakelag.trigger_on_velocity_change },
					//		{ XorStr("On LBY Update"), &g_Vars.fakelag.trigger_on_lby_update },
					//		{ XorStr("While Breaking Lag Comp"), &g_Vars.fakelag.trigger_while_breaking_lc }
					//	};

					//	GUI::Controls::MultiDropdown(XorStr("Alternate Triggers"), alt_fakelag_cond);
					//	GUI::Controls::Slider(XorStr("Alternate Choke"), &g_Vars.fakelag.alternative_choke, 0, 16);
					//}
				...
				GUI::Group::BeginGroup(XorStr("Accuracy"), Vector2D(50, 50)); {
					GUI::Controls::Checkbox(XorStr("Fix Fakelag"), &g_Vars.rage.double_tap_duck);
					...
					GUI::Controls::Checkbox(XorStr("Double Tap") + std::string(XorStr("#") + std::to_string(rage_current_group)), &g_Vars.rage.doubletap);
					GUI::Controls::Hotkey(XorStr("Double Tap key#key") + std::string(XorStr("#") + std::to_string(rage_current_group)), &g_Vars.rage.key_dt);
					GUI::Controls::Slider(XorStr("DT Speed") + std::string(XorStr("#") + std::to_string(rage_current_group)), &g_Vars.rage.double_tap_ticks, 0, 15);

					GUI::Controls::Checkbox(XorStr("Break Lag Compensation") + std::string(XorStr("#") + std::to_string(rage_current_group)), &g_Vars.rage.breaklagcomp);
					GUI::Controls::Hotkey(XorStr("Break LC key#key") + std::string(XorStr("#") + std::to_string(rage_current_group)), &g_Vars.rage.break_key);
					GUI::Controls::Dropdown(XorStr("LC Mode") + std::string(XorStr("#") + std::to_string(rage_current_group)), { XorStr("Shift"), XorStr("Buffer") }, &g_Vars.rage.breaklcmode);
				}
				GUI::Group::EndGroup();
				...
				GUI::Group::BeginGroup(XorStr("Anti aim"), Vector2D(50, 50));
				{
					...
					GUI::Controls::Dropdown(XorStr("LBY Breaker"), { XorStr("Off"), XorStr("Dynamic"), XorStr("Sway"), /*XorStr("Z"),*/ XorStr("Static")}, &settings->yaw);
					if (settings->yaw > 0)
					{
						if (settings->yaw == 4) {
							GUI::Controls::Checkbox(XorStr("Safe Break"), &g_Vars.antiaim.safebreak);
							GUI::Controls::Slider(XorStr("Break Angle"), &g_Vars.antiaim.break_lby, -145, 145);
						}

						GUI::Controls::Checkbox(XorStr("Twist"), &settings->twist);
					}
					...
					GUI::Controls::Checkbox(XorStr("Funny Mode"), &g_Vars.antiaim.funnymode, true);
					GUI::Controls::Checkbox(XorStr("Fake Flick"), &g_Vars.antiaim.fakeflick, true);
					GUI::Controls::Hotkey(XorStr("Fake Flick#Key"), &g_Vars.antiaim.fake_flick);
			...
			if (GUI::Form::BeginTab(2, XorStr("C")) || GUI::ctx->setup) {
				...
				GUI::Group::BeginGroup(XorStr("General"), Vector2D(50, 50));
				{
					...
					GUI::Controls::Dropdown( XorStr( "Hitsound type" ), { XorStr( "Default" ), XorStr("Funny")/*, XorStr("Custom")*/ }, & g_Vars.misc.hitsound_type);
			...
			if (GUI::Form::BeginTab(3, XorStr("D")) || GUI::ctx->setup) {
				...
				GUI::Group::BeginGroup(XorStr("Settings"), Vector2D(50, 40));
				{
					...
					GUI::Controls::Dropdown(XorStr("Cheat Mode"), std::vector<std::string> { XorStr("Spread"), XorStr("No Spread") }, & g_Vars.misc.cheat_mode);
					...
					if (!cfg_list.empty()) {
						...
						GUI::Controls::Button(XorStr("Load config"), [&]() {
							if (selected_cfg <= cfg_list.size() && selected_cfg >= 0) {
								...
								g_Vars.rage.key_dt.enabled = false;
								g_Vars.rage.distortion_resolver.enabled = false;
								g_Vars.rage.key_dmg_override.enabled = false;
								g_Vars.rage.prefer_body.enabled = false;
								g_Vars.rage.override_key.enabled = false;
								g_Vars.antiaim.manual_left_bind.enabled = false;
								g_Vars.antiaim.manual_right_bind.enabled = false;
								g_Vars.antiaim.manual_back_bind.enabled = false;
								g_Vars.misc.extended_backtrack_key.enabled = false;
			...
			if (GUI::Form::BeginTab(4, XorStr("F")) || GUI::ctx->setup) {
